...
void eval_micro_sequencer() {
	...
	unsigned int mar = CURRENT_LATCHES.MAR;
	unsigned int ir = CURRENT_LATCHES.IR;
	int ready = CURRENT_LATCHES.READY;
	int state_number = CURRENT_LATCHES.STATE_NUMBER;
	int ld_pc = get_LD_PC(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_mar = get_LD_MAR(CURRENT_LATCHES.MICROINSTRUCTION);
	int ld_mdr = get_LD_MDR(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_ir = get_LD_IR(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_reg = get_LD_REG(CURRENT_LATCHES.MICROINSTRUCTION);
    int ld_ben = get_LD_BEN(CURRENT_LATCHES.MICROINSTRUCTION);
    int gatepc = get_GatePC(CURRENT_LATCHES.MICROINSTRUCTION);
    int gatemar = get_GateMAR(CURRENT_LATCHES.MICROINSTRUCTION);
    int gatemdr = get_GateMDR(CURRENT_LATCHES.MICROINSTRUCTION);
    int gatealushf = get_GateALUSHF(CURRENT_LATCHES.MICROINSTRUCTION);
    int gaters2 = get_GateRS2(CURRENT_LATCHES.MICROINSTRUCTION);
    int pcmux = get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int addr1mux = get_ADDR1MUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int addr2mux = get_ADDR2MUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int marmux = get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int mdrmux = get_MDRMUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int rs2mux = get_RS2MUX(CURRENT_LATCHES.MICROINSTRUCTION);
    int rs2en = get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION);
    int rs1en = get_RS1En(CURRENT_LATCHES.MICROINSTRUCTION);
    int mio_en = get_MIO_EN(CURRENT_LATCHES.MICROINSTRUCTION);
    int we = get_WE(CURRENT_LATCHES.MICROINSTRUCTION);
    int datasize = get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION);
    int reset = get_RESET(CURRENT_LATCHES.MICROINSTRUCTION);
	
 

	if (ird == 1){
   		NEXT_LATCHES.STATE_NUMBER =(CURRENT_LATCHES.IR << 26);
	}
	else{
			NEXT_LATCHES.STATE_NUMBER = j;
			CURRENT_LATCHES.READY << 2;
	}
    */ 
